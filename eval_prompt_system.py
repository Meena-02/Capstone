...
if not os.path.exists(OUTPUT_IMAGE_FOLDER):
        ...
        print(f"Folder created: {OUTPUT_IMAGE_FOLDER}")
else:
    print(f"Folder already exists: {OUTPUT_IMAGE_FOLDER}")
...
for index, row in df.iterrows():
    ...
    print(f"Ref image 1: {row['ref_img1']}")
    print(f"Ref image 2: {row['ref_img2']}")
    ...
    final_obj = eval.run_image2(runner, vision_model, processor, txt_feats,
                                paddle_ocr, target_img, input_text, ref_imgs)
    
    if final_obj is not None:
        image_name = os.path.basename(row['target_img'])
        final_obj['image'].save(f'{OUTPUT_IMAGE_FOLDER}/{count}_{image_name}')
