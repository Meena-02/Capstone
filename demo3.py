...
def demo(runner, vision_encoder, vision_processor, padding_embed, text_model):
    with gr.Blocks() as demo:
        ...
        def validate_and_run(target_image, input_text, ref_image1, ref_image2, ref_image3):
            ...
            if ref_image1 is None or ref_image2 is None or ref_image3 is None:
                raise gr.Error("Please upload all 3 reference image")
            if not input_text.strip():
                raise gr.Error("Please enter a text prompt")
            
            prompt_images = [ref_image1, ref_image2, ref_image3]
